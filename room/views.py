...
class AddExpense(View):
    ...
    def get(self, request):
        ...
        if get_month is None:
            ...
            get_month = datetime.strptime(get_month, '%Y-%m-%d').date()
            expenses = AddExpenses.objects.filter(date__year=get_month.year).filter(date__month=get_month.month)
            this_month_expense = AddExpenses.objects.filter(date__month=get_month.month).filter(date__year=get_month.year).aggregate(Sum("item_price"))["item_price__sum"]
            x = round(this_month_expense, 2) if this_month_expense is not None else this_month_expense
...
class CalculteExpense(View):
    ...
    def get(self, request):
        ...
        if self.get_month is not None:
            clicked_month["month"] = datetime.strptime(self.get_month, '%Y-%m-%d').date()

        months = {1: "January", 2: "February", 3: "March", 4: "April", 5: "May", 
                    6: "June", 7: "July", 8: "August",
                  9: "September", 10: "October", 11: "November", 12: "December",}
        # given_month = date(date.today().year, months.get(clicked_month.get("month"), date.today().month), 1)
        given_month = clicked_month.get("month", date.today())
        print(given_month, clicked_month)
        ...
        if (len(extract_roommate) != 0) and (len(no_of_days) != 0) and (len(amt_to_paid) != 0) and (clicked_month.get("month") is not None):
            for data in zip(extract_roommate, no_of_days, amt_to_paid):
                ...
                check_month_year = ExpensePaidAmount.objects.filter(month_year=str(months[given_month.month]) + " " + str(given_month.year)).filter(name=person)

                if check_month_year:
                    month_year = str(months[given_month.month]) + " " + str(given_month.year)
                    person = AddRoommate.objects.get(name=data[0])
...
class PubBill(View):
    ...
    def get(self, request):
        ...
        object_id = request.GET.get("obj-id")

        if month is not None and object_id is not None:
            # month_name, obj_id = "".join([ i for i in month if i.isalpha() ]), "".join([ i for i in month if i.isdigit() ])
            print(month, object_id)
            convert_date = datetime.strptime(month, '%Y-%m-%d').date()
            # amt = ExpensePaidAmount.objects.all()
            m = PUBBillAmount.objects.get(id=int(object_id))
            month_year = str(months[convert_date.month]) + " " + str(convert_date.year)
...
class GeneratePDF(View):
    ...
    def get(self, request, pk):
        pub_pdf = PUBBillAmount.objects.get(id=pk)
        data = {
            "pub": pub_pdf,
        }
